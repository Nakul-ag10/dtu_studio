...
export default function PressConferences() {
  ...
  const years = useMemo(() => {
    return Array.from(new Set(videos.map((item) => item.year))).sort((a, b) => b - a);
  }, [videos]);
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          <aside className="rounded-3xl bg-white/90 border border-border p-6 shadow-sm">
            ...
            <p className="mb-6 text-sm text-muted-foreground">
              Current-year conferences are visible by default. Click any year to load its videos.
            </p>
              ...
              {years.map((year) => (
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {videos.filter((item) => item.year === year).length} video{videos.filter((item) => item.year === year).length === 1 ? "" : "s"}
                  </p>
                </button>
              ))}
            </div>
          ...
          <section>
            <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Viewing press conferences for</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredVideos.map((video, index) => (
                <motion.div
                  ...
                  className="bg-white rounded-lg overflow-hidden border border-border hover:shadow-xl transition-all duration-300 cursor-pointer group flex flex-col"
                >
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    ...
                    <h3 className="line-clamp-2 group-hover:text-primary transition-colors flex-grow">
                      {video.title}
                    </h3>
                  </div>
                </motion.div>
